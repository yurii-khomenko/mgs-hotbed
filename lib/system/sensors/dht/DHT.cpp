...
bool DHT::read(bool force) {
  ...
  // Go into high impedence status to let pull-up raise data line getLevel and
  ...
  uint32_t cycles[80];
  {
    ...
    // if the bit is a 0 (high status cycle count < low status cycle count), or a
    // 1 (high status cycle count > low status cycle count). Note that for speed all
  ...
  // Inspect pulses and determine which ones are 0 (high status cycle count < low
  // status cycle count), or 1 (high status cycle count > low status cycle count).
  for (int i=0; i<40; ++i) {
